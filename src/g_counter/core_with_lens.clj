...
(defn create-peer!
  ...
  [shared-state id]
  (let [lens (partial lens/in id)
        self-count-lens (comp lens lens)
        control-ch (chan)
     ...
     (let [[val ch] (alts! [control-ch in timeout-ch])]
       (condp = ch
         ...
         in (do (log/debug "got counter, updating")
                (swap! shared-state
                       #(lens/update %1 lens (partial resolve-conflict %2))
                       val)
                (recur timeout-ch))
